...
```
Il teorema del campionamento stabilisce che un segnale analogico, la cui gamma di frequenza è limitata da una frequenza 
massima fmax il segnale può essere descritto da campioni presi a frequenza fc>2fmax
```
...
import("stdfaust.lib");
process =_*(0.5), _*(0.25), _*(0.125), _*(0.1) <: _,_,_,_,_*(2),_*(4),*(8),_*(10);
```
